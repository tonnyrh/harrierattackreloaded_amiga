...
**Deliberately not changed**: the shared `cpcRandomStateByColumn[]`/`cpcRStateByColumn[]` tables (clouds/flak/town) still index by absolute world column, which the verification work above showed is *also* subtly mis-seeded for the sea section specifically (before land begins) - fixing that would mean anchoring the shared table's known-correct seed to the actual world column it corresponds to and walking the LCG backwards for columns 0 through land's start, a bigger and riskier change than what was asked here. Flagged for a future pass, not fixed now - land's own generator sidesteps the issue entirely by using its own locally-seeded sequence instead of the shared table.
